...
class HyperNetwork(nn.Module):
    def __init__(self, args, hyper_in_features, hyper_hidden_layers, hyper_hidden_features, hypo_module):
        '''
        Args:
        ...
        for name, param in hypo_parameters:
            ...
            hn = FCBlock(in_features=hyper_in_features, out_features=int(torch.prod(torch.tensor(param.size()))),
                                 ...
                                 outermost_linear=True, nonlinearity='relu', use_bias=args.hyp_usebias)
            self.nets.append(hn)
    ...
    def forward(self, z):
        ...
        for name, net, param_shape in zip(self.names, self.nets, self.param_shapes):
            ...
            net_z = net(z)
            params[name] = net_z.reshape(batch_param_shape)
        return params
